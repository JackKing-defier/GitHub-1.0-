...
module.exports = {
    ...
    getRequestOptions2: function (options, page) {
        const that = this;
        options = $.extend({
            method: 'GET', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
            header: {
        ...
        options.delay = options.delay || 500;
        options.data = options.data || {};
        options.data.utoken = wx.getStorageSync("jwtoken");
        options.data.token = _data.duoguan_user_token;
    ...
    mypost: function (url, data, callback, page, options) {
        options = this.getRequestOptions2($.extend({
            url: url,
            data: data,
            callback: callback,
            method: "POST"
